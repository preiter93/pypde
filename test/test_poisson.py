...
class TestPoissonNeumann(unittest.TestCase):
    def setUp(self):
        pass
    ...
    def setUpClass(cls):
        print("----------------------------------")
        print("   Solve Poisson (Neumann 1D)   ")
        print("----------------------------------")
        """ 
        Calculate fftws only once to test solver independently
        """
        super(TestPoissonNeumann, cls).setUpClass()
        cls.N  = N
        cls.u = SpectralField(cls.N+2, "ChebNeumann")
        cls.x = cls.u.x
        cls.f = SpectralField(cls.N+2, "ChebNeumann")

        # Neumann Poisson is singular
        skipfirst = True
        ...
        cls.f.forward()
        fhat = cls.f.vhat
        cls.b = RHSExplicit(f=(B[skip,skip]@fhat[skip]))

        # -- LHS ---
        A1 = MatrixLHS(A[skip,skip],ndim=1,axis=0, solver="tdma")               
        cls.A1 = LHSImplicit(A1)            # Exploit bandedness
    ...
    def test_tdma(self):
        ...
        # -- Solve ---
        self.u.vhat[self.skip] = self.A1.solve(self.b.rhs)
        uhat = self.u.vhat
        norm = np.linalg.norm( uhat-self.solhat )
        print(" |pypde - analytical|: {:5.2e}"
            .format(norm))
...
class TestPoisson2DNeumann(unittest.TestCase):
    ...
    def setUpClass(cls):
        print("----------------------------------")
        print("   Solve Poisson (Neumann 2D)   ")
        print("----------------------------------")
        """ 
        Calculate fftws only once to test solver independently
        """
        ...
        cls.skip = skip
        # -- u
        cls.u = SpectralField(shape, ("CN","CN"))
        cls.x,cls.y = cls.u.x,cls.u.y
        xx, yy = np.meshgrid(cls.x,cls.y,indexing="ij")
        # -- f
        cls.f = SpectralField(shape, ("CN","CN"))
        cls.f.v = cls._f(cls,xx,yy)
        cls.f.forward()
        ...
        # -- RHS ---
        fhat = cls.f.vhat[skip,skip]
        cls.b = RHSExplicit(f=fhat)
        cls.b.add_PM(MatrixRHS(Bx[skip,skip],axis=0))
        cls.b.add_PM(MatrixRHS(Qy.T[skip,skip],axis=1))
    ...
    def test_solve(self):
        ...
        # -- Solve ---
        skip = self.skip
        self.u.vhat[skip,skip] = self.A.solve(self.b.rhs)
        uhat = self.u.vhat
        norm = np.linalg.norm( uhat-self.solhat )
        print(" |pypde - analytical|: {:5.2e}"
            .format(norm))
