...
class NavierStokes(Integrator):
    ...
    def setup_solver(self):
        ...
        for rk in range(self.nstage):
            solver_U = solverplan_hholtz2d_adi(
                bases=self.U.xs, lam=self.dt * self.a[rk] * self.beta * self.nu
            )
            solver_V = solverplan_hholtz2d_adi(
                bases=self.V.xs, lam=self.dt * self.a[rk] * self.beta * self.nu
            )
            solver_T = solverplan_hholtz2d_adi(
                bases=self.T.xs, lam=self.dt * self.a[rk] * self.beta * self.kappa
            )
    ...
    def update_U(self, ux, uz, ux_old, uz_old, p, stage):
        ...
        if self.c[stage] != 0:
            rhs -= self.dt * self.c[stage] * self.conv_term(self.U, ux_old, uz_old)
    ...
    def update_V(self, ux, uz, ux_old, uz_old, That, p, stage):
        ...
        if self.c[stage] != 0:
            rhs -= self.dt * self.c[stage] * self.conv_term(self.V, ux_old, uz_old)
    ...
    def update_T(self, ux, uz, ux_old, uz_old, stage):
        ...
        rhs = (
            -self.dt
            * self.b[stage]
            * self.conv_term(self.T, ux, uz, add_bc=uz * self.bc_dTdz)
        )
        if self.c[stage] != 0:
            rhs -= (
                self.dt
                * self.c[stage]
                * self.conv_term(self.T, ux_old, uz_old, add_bc=uz_old * self.bc_dTdz)
            )
    ...
    def update_pres(self, pres, P, div, stage):
        pres.vhat -= (
            1.0 * self.nu * div * self.beta
        )  # *self.dt*self.a[stage]#self.dt*self.a[stage]*
        pres.vhat += 1.0 / (self.dt * self.a[stage]) * galerkin_to_cheby(P.vhat, P)
