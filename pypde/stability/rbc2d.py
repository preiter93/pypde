...
#--------------------------------------------------------------------
@io_decorator
def solve_rbc2d(Nx=21,Ny=21, Ra=2600, Pr=1,aspect=1, sidewall="adiabatic", 
    plot=True,norm_diff=True):
    '''
    ...
    evals,evecs = solve_stability_2d(Ra,Pr,U,V,T,P,CH,UU,VV,TT,
        scale=(scale_z*aspect,scale_z), norm_diff=norm_diff)
    ...
    if plot:
        ...
        ax.quiver(xx[::skip,::skip],yy[::skip,::skip],
                u[::skip,::skip]/speed,v[::skip,::skip]/speed,
...
def solve_stability_2d(Ra,Pr,U,V,T,P,CH,UU,VV,TT,scale=(1,1),norm_diff=True):
    ...
    D2x = CH.xs[0].dms(2)*scale[0]**2.
    D2y = CH.xs[1].dms(2)*scale[1]**2.

    # Derivative (2D)
    N   = U.vhat.shape[0]*U.vhat.shape[1]; I   = np.eye(N)
    Dx1 = np.kron(D1x,Iy); Dx2 = np.kron(D2x,Iy)
    Dy1 = np.kron(Ix,D1y); Dy2 = np.kron(Ix,D2y)
    ...
    # ------------
    # LHS
    L11 = L2d_u+dUdx  ; L12 = dUdy           ; L13 = dpdx     ; L14 = 0*I
    L21 = dVdx        ; L22 = L2d_v + dVdy   ; L23 = dpdy     ; L24 =-gr*that
    L31 = dudx        ; L32 = dvdy           ; L33 = 0.*I     ; L34 = 0.*I
    L41 = dTdx        ; L42 = dTdy           ; L43 = 0.*I     ; L44 = K2d_t
