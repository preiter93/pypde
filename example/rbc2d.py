...
from example.rbc2d_base import *
...
class NavierStokes(NavierStokesBase,NavierStokesSteadyState,Integrator):
    ...
    def set_temperature(self,amplitude=0.5,m=1):
        self.T.v = amplitude*np.sin(0.5*m*np.pi*self.xx)*np.cos(0.5*np.pi*self.yy)
        self.T.forward()
    ...
    def set_temp_fieldbc(self):
        ...
        self.dTbcdz2 = grad(self.Tbc,deriv=(0,2),return_field=False,L=[self.Lx,self.Lz])
        # First Derivative for Convective term
        vhat = grad(self.Tbc,deriv=(0,1),return_field=False,L=[self.Lx,self.Lz])
        if self.dealias:
    ...
    def setup_solver(self):
        ...
        for rk in range(self.nstage):
            ...
            lam=self.dt*self.a[rk]*self.beta*self.nu, Lx=self.Lx)
            solver_V = solverplan_hholtz2d_adi(bases=self.V.xs,
            lam=self.dt*self.a[rk]*self.beta*self.nu, Lx=self.Lx)
            solver_T = solverplan_hholtz2d_adi(bases=self.T.xs,
            lam=self.dt*self.a[rk]*self.beta*self.kappa, Lx=self.Lx)
    ...
    def divergence_velocity(self,u,v):
        ...
        dudx = grad(u,deriv=(1,0),return_field=False,L=[self.Lx,self.Lz])
        dudz = grad(v,deriv=(0,1),return_field=False,L=[self.Lx,self.Lz])
    ...
    def update_U(self,stage):
        ...
        # Pressure term
        dpdx = grad(self.pres,deriv=(1,0),return_field=False,L=[self.Lx,self.Lz])
        ...
        if self.c[stage] != 0:
            ...
            self.U,self.ux_old,self.uz_old)

        # Add explicit diffusive term
        if self.beta != 1.0:
            rhs += (self.dt*self.a[stage]*(1-self.beta)*self.nu*
            grad(self.U,deriv=(2,0),return_field=False,L=[self.Lx,self.Lz]) )
            rhs += (self.dt*self.a[stage]*(1-self.beta)*self.nu*
            grad(self.U,deriv=(0,2),return_field=False,L=[self.Lx,self.Lz]) )
    ...
    def update_V(self,That,stage):
        ...
        # Pressure term
        dpdz = grad(self.pres,deriv=(0,1),return_field=False,L=[self.Lx,self.Lz])
        ...
        if self.c[stage] != 0:
            rhs -= self.dt*self.c[stage]*self.conv_term(
            self.V,self.ux_old,self.uz_old)
        ...
        if self.beta != 1.0:
            rhs += (self.dt*self.a[stage]*(1-self.beta)*self.nu*
            grad(self.V,deriv=(2,0),return_field=False,L=[self.Lx,self.Lz]) )
            rhs += (self.dt*self.a[stage]*(1-self.beta)*self.nu*
            grad(self.V,deriv=(0,2),return_field=False,L=[self.Lx,self.Lz]) )
    ...
    def update_T(self,stage):
        ...
        # Non-Linear Convection
        rhs = -self.dt*self.b[stage]*self.conv_term(self.T,self.ux,self.uz,
        add_bc = self.uz*self.dTbcdz1)
        if self.c[stage] != 0:
            rhs -= self.dt*self.c[stage]*self.conv_term(
        ...
        if self.beta != 1.0:
            ...
            grad(self.T,deriv=(2,0),return_field=False,L=[self.Lx,self.Lz]) )
            rhs += (self.dt*self.a[stage]*(1-self.beta)*self.kappa*
            grad(self.T,deriv=(0,2),return_field=False,L=[self.Lx,self.Lz]) )
    ...
    def update_pres(self,div,stage):
        ...
        self.pres.vhat +=  1.0/(self.dt*self.a[stage])*galerkin_to_cheby(
        self.P.vhat,self.P)
